...
class QuadrilateralPolynomial {
 public:
   uint_t getNumCoefficients() const
   {
      return basis_.numBasisFunctions();
   }
   ...
   inline std::array<uint_t, Dim> getDegree() const
   {
      return basis_.getDegrees();
   }
   ...
   inline void setZero()
   {
      std::memset(coeffs_.data(), 0, getNumCoefficients() * sizeof(real_t));
   }
